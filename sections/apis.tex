...
\begin{figure}[h]
    ...
    \end{tabular}
    \caption{Gesamtarchitektur der Anwendung}
...
Ein typischer Ablauf innerhalb der Anwendung beginnt mit einer Benutzereingabe im Frontend, etwa einer Filmsuche. Die Suchkomponente übergibt den eingegebenen Titel an einen zuständigen Angular-Service, der daraus eine HTTP-Anfrage an das Backend erzeugt. Das Backend verarbeitet diese Anfrage, ruft bei Bedarf externe Filmdaten über die angebundenen TMDB-Schnittstellen ab, bereitet die Antwort auf und liefert die relevanten Informationen als REST-Antwort an das Frontend zurück. Dort werden die Daten wieder vom Service entgegengenommen, in ein für die Oberfläche geeignetes Format überführt und anschließend in der Ergebnisliste dargestellt. Ein vergleichbarer Ablauf wird auch bei Detailansichten verwendet: Nach Auswahl eines Films wird anhand der Film-ID eine weitere Anfrage ausgelöst, damit zusätzliche Informationen wie Beschreibung, Laufzeit, Genres und verfügbare Anbieter geladen werden können.

Die Trennung zwischen Komponente und Service war dabei für die Wartbarkeit besonders wichtig. Komponenten steuern Benutzerinteraktionen wie Suche, Navigation oder Auswahl eines Films, während Services die eigentliche Kommunikation mit dem Backend bündeln. Dadurch konnten Suchlogik, Detailaufrufe und Providerabfragen an zentraler Stelle verwaltet werden, ohne dieselben HTTP-Abläufe in mehreren Komponenten zu wiederholen. Gleichzeitig ließ sich die Oberfläche dadurch leichter erweitern, weil Datenquellen und Darstellung voneinander entkoppelt blieben.
...
Das betrifft insbesondere Aufrufe zur persönlichen Abo-Verwaltung. Wenn gespeicherte Abonnements geladen, geändert oder neu gespeichert werden, sendet das Frontend die Anfrage nicht nur an den passenden Backend-Endpunkt, sondern überträgt zusätzlich das Bearer-Token im \texttt{Authorization}-Header. Erst dadurch kann das Backend die Anfrage dem aktuell angemeldeten Benutzer zuordnen und ausschließlich dessen Daten verarbeiten. Aus Frontend-Sicht war deshalb nicht nur der reine Datenabruf relevant, sondern das verlässliche Zusammenspiel aus Service-Aufruf, Token-Übergabe und anschließender Aktualisierung der Oberfläche.

\setauthor{AC}
\section{API-Integration externer Filmdatenbanken}
...
Die Filmdaten werden über bereitgestellte Schnittstellen geladen und in Angular-Komponenten eingebunden. Für die Benutzeroberfläche war dabei entscheidend, dass Suchergebnisse übersichtlich dargestellt, Detailinformationen verständlich aufbereitet und Navigationswechsel zwischen Listen- und Detailansicht flüssig umgesetzt werden. Da die Anwendung als Single-Page-Anwendung realisiert wurde, konnte dieser Wechsel ohne vollständigen Seitenneuaufbau erfolgen \cite{mdn_spa}. Als externe Grundlage diente dabei die Dokumentation der TMDB API, insbesondere für Authentifizierung, Filmdaten und Anbieterinformationen \cite{tmdb_getting_started,tmdb_watch_providers}.

\begin{figure}[H]
    \centering
    \includegraphics[width=\textwidth]{pics/Abbildung_2}
    \caption{Anbieteranzeige innerhalb der Film-Detailansicht}
