...
Identitätsprovider Keycloak. Dies stellt sicher, dass die interne
...
kann das Frontend mit einer einzigen Anfrage alle relevanten Informationen
...
alle Verlaufseinträge eines Benutzers sortiert nach dem Zeitpunkt des
...
\subsection{Konfiguration über application.properties}

Die zentrale Konfiguration des Backends erfolgt über die Datei
\path|src/main/resources/application.properties|. Dort werden unter anderem
der TMDB-API-Schlüssel, die Datenbankverbindung über
\path|quarkus.datasource.*| sowie die OIDC- und Keycloak-Parameter über
\path|quarkus.oidc.*| definiert. Zusätzlich werden an dieser Stelle
öffentliche Routen ohne Authentifizierung festgelegt sowie weitere
anwendungsrelevante Einstellungen wie CORS, SQL-Logging und die
Hibernate-Konfiguration verwaltet. Die projektbezogene Bündelung dieser
Angaben erleichtert die Wartung und sorgt dafür, dass Infrastruktur-,
Sicherheits- und Laufzeitparameter zentral nachvollziehbar bleiben.
...
angelegt wird. Der Endpunkt \texttt{PUT /api/users/me/subscriptions} ermöglicht das vollständige Aktualisieren der Abonnements in einer einzigen
Anfrage. Die Verlaufsendpunkte erlauben das Hinzufügen, Abrufen und Löschen
...
Die \texttt{ProviderResource} stellt Endpunkte unter \texttt{/api/providers}
...
Tabelle \ref{tab:backend_rest_endpoints} gibt einen kompakten Überblick über
die zentralen REST-Endpunkte des Backends, deren Zugriffsschutz und ihre
jeweilige Aufgabe.
...
\hline
GET & \path|/api/movies/id/{id}| & öffentlich & Abruf von Filmdaten anhand einer TMDB-ID \\
GET & \path|/api/movies/name/{name}| & öffentlich & Filmsuche über den Titel \\
GET & \path|/api/movies/trending/{window}| & öffentlich & Abruf aktueller Trending-Filme \\
GET & \path|/api/movies/{id}/providers/filtered| & öffentlich & Länderbezogene Provider eines Films \\
GET & \path|/api/movies/{id}/providers/for-user| & geschützt & Providerliste mit Benutzerbezug für den aktuell angemeldeten Benutzer \\
GET & \path|/api/users/me| & geschützt & Abruf des eigenen Benutzerprofils \\
GET & \path|/api/users/me/providers| & geschützt & Abruf aller Provider mit Ownership-Informationen des Benutzers \\
PUT & \path|/api/users/me/subscriptions| & geschützt & Vollständiges Aktualisieren der gespeicherten Abonnements \\
POST & \path|/api/users/me/history| & geschützt & Hinzufügen eines Films zur persönlichen Historie \\
GET & \path|/api/users/me/history| & geschützt & Abruf der persönlichen Historie \\
DELETE & \path|/api/users/me/history/{id}| & geschützt & Löschen eines Eintrags aus der persönlichen Historie \\
GET & \path|/api/providers| & öffentlich & Abruf aller in der Datenbank gespeicherten Provider \\
\hline
\end{tabularx}
\caption{Zentrale REST-Endpunkte des Backends mit Zugriffsschutz}
\label{tab:backend_rest_endpoints}
\end{table}
...
weitergegeben.
Dieses Flag wird im Backend gezielt gesetzt, damit das Frontend abonnierte
Anbieter in der Film-Detailansicht hervorheben und in der Darstellung
priorisiert anordnen kann.
...
Verlaufseinträgen an den Client. Er wird über eine statische
